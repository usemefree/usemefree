...
{
    Services.DownloadPackageInfoService downloadPackageInfoService;
    ...
    public OperatingSystems CoperatingSystems { get; set; }
    public Frm_AddEdit()
    {
        InitializeComponent();
        downloadPackageInfoService = new Services.DownloadPackageInfoService();
    }
    ...
    private void Frm_AddEdit_Load(object sender, EventArgs e)
    {
        ...
        if (info is null)
        {
            ...
            if (CoperatingSystems != null)
                CB_Os.Text = operatingSystems.FirstOrDefault(x => x.id == CoperatingSystems.id).Name ?? "Windows";
        }
        else
        {
    ...
    private void btn_Save_Click(object sender, EventArgs e)
    {
        if (MessageBox.Show("Are you went to sure", "Save", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
        {
            ...
            downloadPackageInfoService.setData(IsEdit, info);
            this.Close();
        }
    }
}
